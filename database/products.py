...
class Products:
    ...
    def get_short(product: Product) -> ShortProductData:
        return ShortProductData(
            ...
            name=product.name, 
            price=product.price, 
            image=Product.parse_images(product.images)[0]
        )
    ...
    def get_detail(sess, product: Product) -> DetailProductData:
        return DetailProductData(
            ...
            name=product.name, 
            info=product.info, 
            rating=Rating.session_get_average_rating(sess, product.id),
            images=Product.parse_images(product.images)
        )
    ...
    @staticmethod
    def session_get_specific_product_detail(sess, product_id: int) -> DetailProductData | None:
